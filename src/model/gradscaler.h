...
class GradScaler {
  ...
  inline auto sum(PerDeviceTensors const &per_device) {
    Type sum = Type(0);
    for (auto &&[_, v] : per_device) sum += v.item<Type>();
    return sum;
  }
 ...
 public:
  ...
  explicit GradScaler(GradScalerOptions const &options = {})
    ...
    if (_enabled && !(torch::cuda::is_available() || torch::hasXLA())) {
      TORCH_WARN(
          "GradScaler is enabled, but CUDA is not available.  Disabling.");
      _enabled = false;
    }
    if (_enabled) {
  ...
  auto _check_scale_growth_tracker(std::string const &funcname) -> void {
    static auto fix =
        "This may indicate your script did not use scaler.scale(loss or "
        "outputs) earlier in the iteration.";
    TORCH_CHECK(_scale.defined(),
                "Attempted " + funcname + " but _scale is None.  " + fix);
    TORCH_CHECK(
        _growth_tracker.defined(),
        "Attempted " + funcname + " but _growth_tracker is None.  " + fix);
  }

  auto _lazy_init_scale_growth_tracker(torch::Device const &dev) -> void {
    TORCH_CHECK(!_growth_tracker.defined(),
                "_growth_tracker initialized before _scale");
    _scale =
        torch::full({1}, _init_scale,
                    c10::TensorOptions().dtype(torch::kFloat32).device(dev));
    _growth_tracker =
        torch::full({1}, _init_growth_tracker,
                    c10::TensorOptions().dtype(torch::kInt32).device(dev));
  }

  template <typename T>
  auto scale(T const &values) {
    if constexpr (std::is_same<T, torch::Tensor>()) {
      if (!_enabled) return values;

      assert(values.is_cuda() || values.device().type() == torch::kXLA);

      if (!_scale.defined()) _lazy_init_scale_growth_tracker(values.device());
    ...
    if constexpr (is_container<T>::value) {
      ...
      const auto apply_scale = [&](auto &&value, auto &&apply_scale) {
        ...
        if constexpr (is_container<Result>::value) {
          ...
          std::transform(
              std::begin(value), std::end(value), std::back_inserter(result),
              [=](auto &&item) { return apply_scale(item, apply_scale); });
          return result;
        }
      };
  ...
  auto _unscale_grads_(std::shared_ptr<torch::optim::Optimizer> optimizer,
                       torch::Tensor &inv_scale, torch::Tensor &found_inf,
                       bool allow_fp16) -> PerDeviceTensors {
    auto per_device_inv_scale = _MultiDeviceReplicator(inv_scale);
    auto per_device_found_inf = _MultiDeviceReplicator(found_inf);

    std::map<c10::DeviceType,
             std::map<c10::ScalarType, std::vector<torch::Tensor>>>
    ...
    for (auto &group : optimizer->param_groups()) {
      for (auto &param : group.params()) {
        ...
        per_device_and_dtype_grads[to_unscale.device().type()]
                                  [to_unscale.dtype().toScalarType()]
                                      .push_back(to_unscale);
      }
  ...
  auto unscale_(std::shared_ptr<torch::optim::Optimizer> optimizer) -> void {
    if (!_enabled) return;
    ...
    auto found_inf = torch::full(
        {1}, 0.0,
        at::TensorOptions().dtype(at::kFloat).device(_scale.device()));

    optimizer_state["found_inf_per_device"] =
        _unscale_grads_(optimizer, inv_scale, found_inf, false);
    optimizer_state["stage"] = OptState::UNSCALED;
  }

  auto _maybe_opt_step(std::shared_ptr<torch::optim::Optimizer> optimizer,
                       OptimizerStates &optimizer_state,
                       torch::optim::Optimizer::LossClosure args)
      -> c10::optional<c10::Scalar> {
    if (optimizer_state.contains("found_inf_per_device")) {
      auto &found_inf_per_device =
          std::get<1>(optimizer_state["found_inf_per_device"]);
      if (!sum(found_inf_per_device)) {
        auto tensor = optimizer->step(args);
        if (tensor.defined()) return tensor.item();
      }
    }
    return c10::nullopt;
  }

  auto step(std::shared_ptr<torch::optim::Optimizer> optimizer,
            torch::optim::Optimizer::LossClosure optimizer_args = nullptr)
      -> c10::optional<c10::Scalar> {
    if (!_enabled) {
    ...
    // if getattr(optimizer, "_step_supports_amp_scaling", False):
    {
      ...
      // TODO: Future Feature
      // ------------------------------------------------------------------------
      // The boolean '_step_supports_amp_scaling' force to use dynamic
      // inspection of class signature (thank's to python reflection) to call
      // step() with with extra parameters (two tensors : 'grad_scale' &
      // 'found_inf')
      // ------------------------------------------------------------------------
      // Because step() method in torch::optim::Optimizer() class is pure
      // virtual and fixed to one single parameter (LossClosure functor). It's
      // impossible currently to mimic the current python's behavior of the
      // extra parameters
      // ------------------------------------------------------------------------
    }

    if (optimizer_state["stage"] == OptState::READY) unscale_(optimizer);

    auto &&found_inf_per_device =
        std::get<1>(optimizer_state["found_inf_per_device"]);
    TORCH_CHECK(found_inf_per_device.size() > 0,
  ...
  void update(c10::optional<std::variant<double, torch::Tensor>> const
                  &new_scale = c10::nullopt) {
    if (!_enabled) return;
    ...
    if (new_scale.has_value()) {
      ...
      std::visit(
          [=, this](auto &&arg) {
            ...
            if constexpr (std::is_same_v<T, double>) {
              ...
              static auto reason =
                  "new_scale should be a float or a 1-element "
                  "torch.cuda.FloatTensor with requires_grad=False.";
      ...
      assert(found_infs.size() >
             0);  // No inf checks were recorded prior to update
      ...
      torch::_amp_update_scale_(_scale, _growth_tracker, found_inf_combined,
  ...
  torch::Tensor _get_scale_async() const { return _scale; }
  ...
  double get_growth_factor() const { return _growth_factor; }

  void set_growth_factor(double new_factor) { _growth_factor = new_factor; }

  double get_backoff_factor() const { return _backoff_factor; }

  void set_backoff_factor(double new_factor) { _backoff_factor = new_factor; }

  int64_t get_growth_interval() const { return _growth_interval; }

  void set_growth_interval(int64_t new_interval) {
    _growth_interval = new_interval;
  }

  int64_t get_init_growth_tracker() const { return _init_growth_tracker; }
 ...
 private:
  ...
  inline Type read(torch::serialize::InputArchive &archive,
 ...
 public:
  ...
  void load(torch::serialize::InputArchive &archive) {
    ...
    if (archive.try_read("gradscaler", state)) {
      ...
      if (_scale.defined()) _scale.fill_(_init_scale);
 ...
 private:
  ...
  inline OptimizerStates &get_per_optimizer_states(
      std::uintptr_t optimizer_id) {
 ...
 private:
  class _MultiDeviceReplicator {
   public:
    explicit _MultiDeviceReplicator(torch::Tensor &master_tensor)
        : master(master_tensor) {
      assert(master_tensor.is_cuda() ||
             master_tensor.device().type() == torch::DeviceType::XLA);
    }
...
serialize::OutputArchive &operator<<(serialize::OutputArchive &archive,
                                     const GradScaler &scaler) {
  scaler.save(archive);
  return archive;
}
